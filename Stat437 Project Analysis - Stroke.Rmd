...
for(i in 1:length(stroke$work_type)) {
        if(stroke$work_type[i] == "Self-employed") {
                stroke$work_type[i] = 1
        } else {
                stroke$work_type[i] = 0
        }
}
...
# Create Table 1
```{r}
table1 <- CreateTableOne(vars = xvars, strata = "work_type", data = stroke, test = FALSE)
